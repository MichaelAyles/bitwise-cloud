    ...
    <div>
        ...
        <form onSubmit={handleCreate} className="bg-slate-800/50 border border-slate-700/50 rounded-lg p-4 mb-6">
          ...
          <div className="mb-4">
            ...
            <input type="text" required value={name} onChange={e => setName(e.target.value)}
              ...
              className="w-full max-w-sm bg-slate-700/50 border border-slate-600/50 rounded px-3 py-2 text-white text-sm focus:outline-none focus:border-blue-500" />
          </div>
          {docs.length > 0 && (
            <div className="mb-4">
              <label className="block text-sm text-slate-400 mb-2">Document access</label>
                  ...
                  <button key={doc.id} type="button" onClick={() => toggleDoc(doc.id)}
                    className={`text-xs px-2 py-1 rounded border transition-colors ${
                      selectedDocs.includes(doc.id)
                        ...
                        : 'bg-slate-700/50 border-slate-600/50 text-slate-400 hover:border-slate-400'
                    }`}>
          ...
          <div className="flex gap-2">
            ...
            <button type="button" onClick={() => setShowCreate(false)}
              className="bg-slate-700/50 hover:bg-slate-600/50 text-slate-300 text-sm rounded px-4 py-2 transition-colors">
              Cancel
            </button>
          </div>
        </form>
      ...
      {keys.length === 0 && !showCreate ? (
        <div className="border border-dashed border-slate-600/50 rounded-lg p-12 text-center">
          <p className="text-slate-400 mb-2">No API keys</p>
          <p className="text-slate-500 text-sm">Create an API key to use with MCP tools or the REST API</p>
        </div>
      ) : (
