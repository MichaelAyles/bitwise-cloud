...
export default function Login() {
  ...
  const handleOAuthToken = useCallback(async (token: string) => {
    ...
    setError('');
    try {
      const { access_token } = await auth.oauthShoo(token);
  ...
  useEffect(() => {
    if (!shooLoading && identity?.token) {
      handleOAuthToken(identity.token);
    }
  }, [shooLoading, identity, handleOAuthToken]);
    ...
    <div className="min-h-screen flex items-center justify-center bg-zinc-950 px-4">
      <div className="w-full max-w-sm">
        ...
        <button
          type="button"
          onClick={() => signIn()}
          disabled={oauthLoading || shooLoading}
