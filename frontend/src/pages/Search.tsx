...
export default function Search() {
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSearch} className="mb-6">
        <div className="flex gap-2">
          <input type="text" value={query} onChange={e => setQuery(e.target.value)}
            ...
            className="flex-1 bg-zinc-900 border border-zinc-700 rounded px-3 py-2 text-white text-sm focus:outline-none focus:border-blue-500" />
        ...
        {docs.length > 1 && (
          <div className="flex flex-wrap gap-2 mt-3">
            <span className="text-xs text-zinc-500 py-1">Filter:</span>
            {docs.map(doc => (
      ...
      {searching && <p className="text-zinc-400 text-sm">Searching...</p>}
            ...
            <div key={i} className="bg-zinc-900 border border-zinc-800 rounded-lg p-4">
              <div className="flex items-start justify-between mb-2">
                <div>
                  ...
                  {r.section && <span className="text-xs text-zinc-500 ml-2">&gt; {r.section}</span>}
                </div>
